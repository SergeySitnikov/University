...
public class Main {
    public static void main(String[] args) {
        DoubleUnaryOperator function = x -> Math.atan(x) - 1 / (3 * Math.pow(x, 3));
        DoubleUnaryOperator derivative = x -> 1 / (1 + x * x) + 1 / Math.pow(x, 4);
        ...
        switch (choice) {
            case 0:
                ...
                derivative = x -> 1 / (1 + x * x) + 1 / Math.pow(x, 4);
                Xs1[0] = -1.5;
                h = new HalfDivision(function, derivative, Xs1);
                break;
            case 1:
                function = x -> Math.pow(x, 4) - x - 1;
                derivative = x -> 3 * Math.pow(x, 3) - 1;
                Xs1[0] = 0.693;
                h = new HalfDivision(function, derivative, Xs1);
                break;
            case 2:
                function = x -> (x - 1) * (x - 1) * Math.pow(2, x) - 1;
                derivative = x -> 2 * (x - 1) * Math.pow(2, x) + (x - 1) * (x - 1) * Math.pow(2, x) * Math.log(2);
                Xs2[0] = -1.885;
                Xs2[1] = 1;
                h = new HalfDivision(function, derivative, Xs2);
                break;
            case 3:
                function = x -> Math.pow(Math.tan(x), 3) - x + 1;
                derivative = x -> 3 * Math.pow(Math.tan(x), 2) / (Math.pow(Math.cos(x), 2)) - 1;
                Xs2[0] = -0.47;
                Xs2[1] = 0.47;
                h = new HalfDivision(function, derivative, Xs2);
                break;
        ...
        h.solve();
        String[] names = {"arctg(x)- 1/(3*x^3)", "x^4 - x - 1", "(x-1)^2*2^x - 1", "tg^3(x)", " 1/(3*x^3)", "x-1"};
